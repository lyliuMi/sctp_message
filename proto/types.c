...
char *ogs_plmn_id_to_string(const ogs_plmn_id_t *plmn_id, char *buf)
{
    ...
    if (ogs_plmn_id_mnc_len(plmn_id) == 2)
        log_snprintf(buf, OGS_PLMNIDSTRLEN, "%03d%02d",
                ogs_plmn_id_mcc(plmn_id), ogs_plmn_id_mnc(plmn_id));
    else
        log_snprintf(buf, OGS_PLMNIDSTRLEN, "%03d%03d",
                ogs_plmn_id_mcc(plmn_id), ogs_plmn_id_mnc(plmn_id));
...
uint16_t ogs_plmn_id_mcc_from_fqdn(char *fqdn)
{
    ...
    if (p == NULL) {
        // //("Invalid FQDN [%d:%s]", (int)strlen(fqdn), fqdn);
        return 0;
    }
...
char *ogs_id_get_type(const char *str)
{
    ...
    T = (char*)malloc(strlen(token)+1);\
    memcpy(T, token, strlen(token));\
    type = T;
    if (!type) {
        // //("strdup[%s:%s] failed", str, token);
        goto cleanup;
    }
...
int ogs_fqdn_parse(char *dst, const char *src, int length)
{
    ...
    while (i+1 <= length) {
        len = src[i++];
        if ((j + len + 1) > length) {
            // //("Invalid FQDN encoding[j:%d+len:%d] + 1 > length[%d]",
                    // j, len, length);
...
int ogs_pco_parse(ogs_pco_t *pco, unsigned char *data, int data_len)
{
    ...
    while(size < data_len && i < OGS_MAX_NUM_OF_PROTOCOL_OR_CONTAINER_ID) {
        ...
        assert(size + sizeof(id->id) <= data_len);
        memcpy(&id->id, data + size, sizeof(id->id));
        id->id = be16toh(id->id);
        size += sizeof(id->id);

        assert(size + sizeof(id->len) <= data_len);
...
int ogs_pco_build(unsigned char *data, int data_len, ogs_pco_t *pco)
{
    ...
    assert(size + 1 <= data_len);
    memcpy(data + size, &target, 1);
    size += 1;

    assert(target.num_of_id <= OGS_MAX_NUM_OF_PROTOCOL_OR_CONTAINER_ID);
...
//     addr = calloc(1, sizeof(ogs_sockaddr_t));
//     if (!addr) {
//         //("calloc() failed");
//         return -1;
//     }
...
//     addr6 = calloc(1, sizeof(ogs_sockaddr_t));
//     if (!addr6) {
//         //("calloc() failed");
//         free(addr);
//         return -1;
...
//         //("Invalid IPv6 Prefix string = %s", v);
//         free(v);
//         return -1;
//     }
...
void ogs_subscription_data_free(ogs_subscription_data_t *subscription_data)
{
    ...
    if (subscription_data->imsi)
        free(subscription_data->imsi);
    if (subscription_data->mme_host)
        free(subscription_data->mme_host);
    if (subscription_data->mme_realm)
        free(subscription_data->mme_realm);
    ...
    for (i = 0; i < subscription_data->num_of_slice; i++) {
        ...
        for (j = 0; j < slice_data->num_of_session; j++) {
            if (slice_data->session[j].name)
                free(slice_data->session[j].name);
        }
...
static int flow_rx_to_gx(ogs_flow_t *rx_flow, ogs_flow_t *gx_flow)
{
    ...
    if (!strncmp(rx_flow->description,
        ...
        char* T = (char*)malloc(strlen(rx_flow->description)+1);\
        memcpy(T, rx_flow->description, strlen(rx_flow->description));\
        gx_flow->description = T;
        assert(gx_flow->description);
        ...
        gx_flow->description = calloc(1, len);
        assert(gx_flow->description);
        ...
        assert(len == strlen(gx_flow->description)+1);
    } else {
        // //("Invalid Flow Descripton : [%s]", rx_flow->description);
        return -1;
    }
