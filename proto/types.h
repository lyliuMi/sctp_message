...
#define OGS_STORE_PCC_RULE(__dST, __sRC) \
    do { \
        ...
        if ((__sRC)->name) { \
            (__dST)->name = strdup((__sRC)->name); \
            assert((__dST)->name); \
        } \
        if ((__sRC)->id) { \
            (__dST)->id = strdup((__sRC)->id); \
            assert((__dST)->id); \
        } \
        for (__iNDEX = 0; __iNDEX < (__sRC)->num_of_flow; __iNDEX++) { \
            ...
            (__dST)->flow[__iNDEX].description = \
                strdup((__sRC)->flow[__iNDEX].description);  \
            assert((__dST)->flow[__iNDEX].description); \
        } \
...
#define OGS_STORE_SESSION_DATA(__dST, __sRC) \
    do { \
        ...
        if ((__sRC)->session.name) { \
            (__dST)->session.name = strdup((__sRC)->session.name); \
            assert((__dST)->session.name); \
        } \
...
#define OGS_SESSION_DATA_FREE(__sESSdATA) \
    do { \
        int i; \
        assert((__sESSdATA) != NULL); \
        if ((__sESSdATA)->session.name) \
            free((__sESSdATA)->session.name); \
